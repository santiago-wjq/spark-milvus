...
/** One query's candidates as bytes: what a first-stage task sends on.
  ...
  * than that many JVM objects, and lets the merge stage join two tasks'
  * answers by walking both once
  * (docs/design/architecture/vector-search.html sections 2.1 and 2.6).
  *
...
object CandidateBytes {
  ...
  /** The best k of two packed answers to the same query, best first.
    ...
    * or merging three answers two at a time in any order, gives the same
    * bytes: the ranking is total, which is what the two tie-breakers in
  ...
  def merge(
    ...
    if (leftSize == 0) return if (rightSize <= k) right else right.take(k * Width)
