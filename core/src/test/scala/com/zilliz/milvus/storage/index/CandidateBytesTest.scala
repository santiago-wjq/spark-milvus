...
class CandidateBytesTest extends AnyFunSuite with Matchers {
  ...
  test("packing sorts by the metric, best first") {
    val given =
      Seq(candidate(101L, 1L, 3.0), candidate(102L, 2L, 1.0), candidate(103L, 3L, 2.0))
  ...
  test("a merge keeps the best k and drops the rest") {
    val left =
      CandidateBytes.of(Seq(candidate(101L, 1L, 1.0), candidate(101L, 2L, 4.0)), "L2")
    val right =
      CandidateBytes.of(Seq(candidate(102L, 3L, 2.0), candidate(102L, 4L, 5.0)), "L2")
  ...
  test("merging packed answers agrees with merging candidates, in any order") {
    ...
    Seq("L2", "COSINE").foreach { metric =>
      ...
      val straight = places(packed.reduce(CandidateBytes.merge(_, _, 6, metric)))
  ...
  test("a width that is not a candidate is refused") {
    ...
    the[IllegalArgumentException] thrownBy CandidateBytes.of(Seq.empty, "EUCLID")
  }
}
